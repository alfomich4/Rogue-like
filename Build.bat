...
set MSBUILD_PATH="E:\Program Files\Microsoft Visual Studio\2022\Community\MSBuild\Current\Bin\MSBuild.exe"
...
set "RES_SRC1=%CD%\XYZRoguelike\Resources"
set "RES_DST=%CD%\%PLATFORM%\%CONFIGURATION%\Resources"

if exist "%RES_SRC1%\" (
    mkdir "%RES_DST%" >nul
    xcopy /E /I /Y "%RES_SRC1%\*" "%RES_DST%\" >nul
    echo Copied resource files from XYZRoguelike\Resources to output directory.
...
set "TARGET=XYZRoguelike.exe"
set "EXE_PATH=%CD%\%PLATFORM%\%CONFIGURATION%\%TARGET%"
if not exist "%EXE_PATH%" (
    set "EXE_PATH=%CD%\XYZRoguelike\%PLATFORM%\%CONFIGURATION%\%TARGET%"
)
if not exist "%EXE_PATH%" (
